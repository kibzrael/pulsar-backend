...
class UserSerializer(serializers.ModelSerializer):
    ...
    followers = serializers.IntegerField(
        source='followedId.count', read_only=True)
    posts = serializers.IntegerField(source='user_post.count', read_only=True)

    is_following = serializers.SerializerMethodField(
        method_name='is_following_method')

    def is_following_method(self, instance):
        request_user_id = self.context.get('request_user_id')
        try:
            return Follow.objects.filter(user=instance, follower__id=request_user_id).exists()
        except:
            return False

    class Meta:
        model = User
        fields = ['id', 'username', 'category', 'profile_pic', 'fullname', 'email', 'bio',
                  'portfolio', 'is_following', 'posts', 'followers', 'date_of_birth', 'date_joined', 'is_superuser']
...
class MinimalUserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'username', 'category',
                  'profile_pic', 'fullname', 'is_following']