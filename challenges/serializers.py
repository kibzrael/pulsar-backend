...
class ChallengeSerializer(serializers.ModelSerializer):
    ...
    def is_pinned_method(self, instance):
        ...
        try:
            return Pin.objects.filter(challenge=instance, user__id=request_user_id).exists()
        except:
            return False
    class Meta:
        model = Challenge
        fields = ['id','name','category','description','cover','is_pinned','pins','posts','value','time_created']
